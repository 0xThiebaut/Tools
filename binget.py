...
try:
    ...
    print(
        "Did you install the requests module? See https://pypi.org/project/requests/",
        file=stderr,
    )
    raise
...
try:
    ...
    print(
        "Did you install the python-dateutil module? See https://pypi.org/project/python-dateutil/",
        file=stderr,
    )
    raise
                ...
                yield Result(
                    ...
                    deep_links=[
                        DeepLink(
                            ...
                            snippet=deeplink.get("snippet"),
                        )
                        for deeplink in entry.get("deepLinks", [])
                    ],
                )
                if size is not None:
                    size -= 1
...
def download(url: str, prefix: str = "", suffix: str = ""):
    ...
    date = (
        ...
        if "Last-Modified" in response.headers
            ...
            randint(int(parse("2019-01-01").timestamp()), int(time()))
        )
    )
    ...
    with NamedTemporaryFile(
        mode="wb", delete=False, dir=".\\", prefix=prefix, suffix=suffix
    ) as file:
        name = file.name
...
For more information on:
    ...
    parser.add_argument(
        "query", metavar="QUERY", type=str, help="the Bing query to download"
    )
    parser.add_argument(
        "--key",
        metavar="KEY",
        type=str,
        default=environ.get("BING_SEARCH_V7_SUBSCRIPTION_KEY"),
